...
const connMap = new Map<WebSocket, { roomCode: string; peerId: string }>();
...
wss.on('connection', (ws: WebSocket) => {
  ws.on('message', (raw: Buffer) => {
    let msg: { type: string; room?: string; nickname?: string; peerId?: string; data?: unknown };
    try { msg = JSON.parse(raw.toString()); } catch { sendErr(ws, 'Invalid JSON'); return; }
    ...
    switch (msg.type) {
      ...
      case 'create': {
        ...
        const info: PeerInfo = { ws, peerId, nickname: msg.nickname || 'Anonymous' };
      ...
      case 'join': {
        ...
        if (!code) { sendErr(ws, 'Room code required'); return; }
        const room = rooms.get(code);
        if (!room) { sendErr(ws, 'Room not found'); return; }

        const peerId = genPeerId();
        const nickname = msg.nickname || 'Anonymous';
        const info: PeerInfo = { ws, peerId, nickname };
      ...
      case 'signal': {
        ...
        if (!conn) { sendErr(ws, 'Not in a room'); return; }
        const room = rooms.get(conn.roomCode);
        if (!room) { sendErr(ws, 'Room vanished'); return; }

        const data = msg.data as { targetPeerId?: string; description?: unknown; candidate?: unknown } | undefined;
      ...
      case 'update_nickname': {
        ...
        if (info) info.nickname = msg.nickname || info.nickname;
        break;
      }
...
function sendErr(ws: WebSocket, msg: string): void {
